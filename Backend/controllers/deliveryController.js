...
exports.createDelivery = async (req, res) => {
    try {
        ...
        sms(pickupMob); // Ensure this function is defined and works correctly

        res.status(201).json({ // 201 Created is more appropriate for a POST request
            success: true,
            delivery,
        });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: err.message }); // Send error response to client
    }
};
