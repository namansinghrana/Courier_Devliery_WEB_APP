...
const Dashy = () => {
  ...
  useEffect(() => {
    const fetchCountDelivery = async () => {
      try {
        const response = await fetch('/api/v1/delivery/countDelivery', {
          method: 'GET'
        });
    ...
    const fetchGetCount = async () => {
      try {
        const response = await fetch('/api/v1/count/get-count', {
          method: 'GET'
        });
        const data = await response.json();
    ...
    const fetchMyOrder = async () => {
      try {
        const response = await fetch('/api/v1/delivery/myorder', {
          method: 'GET'
        });
        const data = await response.json();
        setTable(data.order);
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/delivery/getMonthWise/${curryear}/month-wise`, {
      method: "GET"
    })
      .then(res => res.json())
      .then(res => {
        ...
        <div className='col-span-4 pe-9 pt-10 '>
          ...
          <div className='grid grid-cols-3 gap-6 mt-12 h-44 drop-shadow-lg'>
            ...
            <div className='bg-gradient-to-r from-rose-300 to-fuchsia-50 rounded-lg'>
              ...
              <div className='container mt-3 ps-12'>
                <div className='flex flex-row space-x-36'>
                  <p className='font-light text-7xl'>{cancel || '6'}</p>
